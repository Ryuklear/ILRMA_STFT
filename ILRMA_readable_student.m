function [Y, cost, W] = ILRMA_readable_student(X, type, It, nb, drawConv, normalize, W, T, V, Z)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
P = max(abs(Y).^2,eps); %推定音源Yパワースペクトログラム
E = eye(N);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% ILRMA1 %%%%%%%%%%%%%%%%%%%%%%%%%%%
if (type==1) % Algorithm for ILRMA1_student-t distribution
    for n=1:N
        ...
        for n=1:N
            %%%%%Update W and Y%%%%%
             for i=1:I
                ...
                alpha(i,:,n) = ones(1,J)+(2/nu) * P(i,:,n)./(R(i,:,n).^(2/p));
                
                D = (2/nu + 1)* ((Xp(:,:,i).*(UN1*(U1J./(alpha(i,:,n).*(R(i,:,n).^(2/p))))))*Xp(:,:,i)')/J;
                w = (W(:,:,i)*D)\E(:,n); % M x 1
...
%              R(:,:,n) = T(:,:,n)*V(:,:,n);
            ...
            T(:,:,n) = T(:,:,n).*((P(:,:,n).*((nu/(nu+2))*R(:,:,n).^(2/p)+(2/(nu+2))*P(:,:,n)).^(-1).*R(:,:,n).^(-1)*V(:,:,n)')./...
            (R(:,:,n).^(-1)*V(:,:,n)')).^(p/(p+2));
            T(:,:,n) = max(T(:,:,n),eps);
            R(:,:,n) = T(:,:,n)*V(:,:,n);
...
%             R(:,:,n) = T(:,:,n)*V(:,:,n);
            ...
            V(:,:,n) = V(:,:,n).*((T(:,:,n)'*(P(:,:,n).*((nu/(nu+2))*R(:,:,n).^(2/p)+(2/(nu+2))*P(:,:,n)).^(-1).*R(:,:,n).^(-1)))./...
            (T(:,:,n)'*R(:,:,n).^(-1))).^(p/(p+2));
            V(:,:,n) = max(V(:,:,n), eps);
            R(:,:,n) = T(:,:,n)*V(:,:,n);
        ...
        if normalize
            ...
            lambdaIJ = repmat(lambda,[I,J,1]).^p; % I x J x N
            R = R./lambdaIJ; % I x J x N
            lambdaIL = repmat(lambda,[I,L,1]).^p; % I x L x N
